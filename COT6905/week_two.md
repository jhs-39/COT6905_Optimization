...
Negative example: the positive quadrant of the x-y plane is NOT a space. Subtract $[1,1] - [2,2] = [-1,-1]$ -> we can enter the negative quadrant!

How do we properly define a subspace?
A subspace must also be closed under vector addition and scalar multiplication. The result is that the subspace MUST include the origin of the original space (otherwise, multiplication by scalar 0 will move us off the subspace, violating closure of the subspace).
